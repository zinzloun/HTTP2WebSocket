...
class proxyServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.post_body = str(
            unquote(self.post_body.decode('utf-8')))  # URL decode
        # Strip of the artificial parameter
        self.post_body = self.post_body[len(PARAMETER):]
        if PARAMETER != '' and self.post_body[:len(PARAMETER)] == PARAMETER:
            # Strip off last & if parameter provided
            self.post_body = self.post_body[:-
                                            1] if '&' == self.post_body[-1] else self.post_body
    ...
    def http_response(self, response):
        ...
        self.end_headers()
        try:
            # Print response in the console too
    ...
    def ws_request(self, url, content):
        ...
        if 'ws://' != target[:5] and 'wss://' != target[:6]:
            error = 'Protocol not supported. Target {} has to have ws:// or wss:// protocol specified'.format(
                target)
            print('[-] {}'.format(error))
            return {'error': error}
        ...
        if CONNECTION is None or not CONNECTION.connected:
            try:
                if proxy:
                    CONNECTION = create_connection('{}{}'.format(
                        target, url), http_proxy_host=proxy[0], http_proxy_port=proxy[1], sslopt=context, header=headers, timeout=WAIT)
                    CONN_START = datetime.now()
                    sleep(DELAY)
                else:
                    CONNECTION = create_connection('{}{}'.format(
                        target, url), sslopt=context, header=headers, timeout=WAIT)
                    CONN_START = datetime.now()
                    sleep(DELAY)
    ...
    def log_message(self, log_format, *args):
        print('{} - - [{}] {}'.format(self.address_string(),
              self.log_date_time_string(), log_format % args))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='HTTP2WebSocket is Python 3 Web Socket Proxy tool to translate HTTP traffic to WebSocket application. It allows to test WebSocket app with tools like sqlmap, dirb, commix. Headers sent by you will be passed to Web Socket application.')
    parser.add_argument(
        '-l', '--listen', help='Local port to listen to.', type=int, required=True)
    parser.add_argument(
        '-t', '--target', help='Your target WebSocket application (E.g.: ws://localhost:1234)')
    parser.add_argument(
        '-w', '--wait', help='Wait specific amount of seconds for WS responses', type=int, default=1)
    parser.add_argument(
        '-K', '--keep', help='Keep the connection open for provided amount of seconds', type=int, default=3)
    parser.add_argument(
        '-d', '--delay', help='The delay between opening a WS connection and sending a request', type=int, default=0)
    ...
    parser.add_argument('-v', '--verbose', default=False,
                        action='store_true', help='Shows HTTP POST body message.')
    parser.add_argument('-k', '--insecure', default=False,
                        action='store_true', help='Don\'t verify SSL certificate.')
